...
namespace eosiobios {
   ...
   /**
    * The `amax.bios` is the first sample of system contract provided by `Armoniax` through the AMAX platform. It is a minimalist system contract because it only supplies the actions that are absolutely critical to bootstrap a chain and nothing more. This allows for a chain agnostic approach to bootstrapping a chain.
    * 
