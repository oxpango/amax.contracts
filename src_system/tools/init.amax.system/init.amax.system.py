...
def regProducers(b, e, is_new = False):
    for i in range(b, e):
        a = accounts[i]
        url = 'https://' + a['name'] + '.com' + '/' + a['pub']

        if is_new:
            params =  jsonArg([a['name'], a['pub'], url, 0, 8000])
        else:
            params =  jsonArg([a['name'], a['pub'], url, 0])

        retry(args.amcli + 'push action amax regproducer ' + params + ' -p ' + a['name'] + '@active')
...
def stepVote():
    ...
    listProducers()
    sleep(5)
def stepUpgradeContracts():
    retry(args.amcli + 'set contract amax ' + args.new_contracts_dir + '/amax.system/')
    retry(args.amcli + 'set contract amax.reward ' + args.new_contracts_dir + '/amax.reward/')
    retry(args.amcli + 'set account permission amax.reward active --add-code')
    # update all producers
    regProducers(firstProducer, firstProducer + numProducers, True)
...
def upgradeVote():
    retry(args.amcli + 'push action amax upgradevote "[]" -p amax@active')
    sleep(1)
    now = datetime.utcnow()
...
commands = [
    ...
    ('U', 'upgrade-contract',   stepUpgradeContracts,       True,    "Upgrade new version contracts"),
    ('',  'addvote',            stepAddVote,                True,    "Vote for producers"),
    ('',  'vote-new',           stepVoteNew,                True,    "Vote for producers by new strategy"),
    ('',  'upgrade-vote',       upgradeVote,                True,    "Upgrade vote"),
...
parser.add_argument('--new-contracts-dir', metavar='', help="Path to the new contracts directory", default='${HOME}/amax/amax.contracts/0.6/contracts')
